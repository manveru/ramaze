...
class SpecLayout < Ramaze::Controller
  map '/'
  layout 'wrapper'
...
class SpecOtherLayout < Ramaze::Controller
  map '/other'
...
class SpecSubLayout < Ramaze::Controller
  map '/sub'
  layout 'sub_wrapper'
...
class SpecLayoutHaml < Ramaze::Controller
  map '/with_haml'
  layout 'wrapper'
  provide :html => :haml
...
describe 'Action rendering' do
  ...
  it 'should work with haml layout' do
    get('/with_haml').body.should == "haml\n with layout.\n"
    get('/with_haml/foo').body.should == "bar\n with layout.\n"
  end
end
